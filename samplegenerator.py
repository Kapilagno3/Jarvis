...
#     faces=detector.detectMultiScale(convertedImage,1.3,5)

#     for (x,y,w,h)in faces:

#         cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)#to draw rect on an image
#         count+=1

#         cv2.imwrite("samples/face."+str(face_id)+"."+str(count)+".jpg",convertedImage[y:y+h,x:x+w])
#         #to capture and save images in database

#         cv2.imshow("image",img)#display image in a window
#     k=cv2.waitKey(100)& 0xff #waits for key pressed
#     if k==27:#press esc to escape
#         break
#     elif count>=10: #take number of samples more the merrier
#         break
